...
def get_conditions(
    type: str | None = Query(None, description="Filtrar por tipo: 'PATHOLOGY' o 'DISEASE'"),
...
def get_methods(
    category: str | None = Query(None, description="Filtrar por categoría: 'FORCE' o 'RESISTANCE'"),
    db: Session = Depends(deps.get_db)
) -> list[MethodSchema]:
...
def get_excersices(
    name: str | None = Query(None, description="Buscar por nombre o término (ej: 'Bench Press', 'Push-up')"),
    muscle_group: str | None = Query(None, description="Filtrar por grupo muscular (ej: 'Pecho', 'Pierna')"),
    pattern: str | None = Query(None, description="Filtrar por patrón de movimiento"),
    level: str | None = Query(None, description="Filtrar por nivel sugerido (ej: 'Intermedio')"),
    goal_code: str | None = Query(None, description="Filtrar por código de objetivo (ej: 'PG')"),
    exclude_conditions: list[str] | None = Query(None, alias="exclude_conditions", description="Códigos de condiciones médicas a excluir (ej: ['PAT002'])"),
    db: Session = Depends(deps.get_db)
