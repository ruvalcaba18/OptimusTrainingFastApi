...
class AssessmentSeeder(BaseSeeder):
    ...
    def seed_session_durations(self):
        ...
        data = [
            (
                "EXPRESS",
                "Express (15-30 min)",
                "Sesiones rápidas y de alta intensidad.",
            ),
            (
                "STANDARD",
                "Estándar (45-60 min)",
                "El tiempo ideal para un entrenamiento completo.",
            ),
            (
                "EXTENDED",
                "Extendido (90+ min)",
                "Para quienes disfrutan de sesiones largas y detalladas.",
            ),
            (
                "VARIABLE",
                "Variable",
                "Mi tiempo cambia cada día y necesito flexibilidad.",
            ),
        ]
        for code, name, description in data:
            self.session.add(
                SessionDuration(code=code, name=name, description=description)
            )
        self.session.commit()
    ...
    def seed_workout_places(self):
        ...
        data = [
            (
                "gimnasio",
                "Gimnasio completo",
                "Tengo acceso a máquinas, pesas y racks.",
            ),
            ("casa", "En casa", "Entrenaré en mi hogar, con o sin equipo."),
            (
                "afuera",
                "Al aire libre",
                "Prefiero parques, pistas o barras al aire libre.",
            ),
            ("mixto", "Híbrido", "Combinaré diferentes lugares (casa, gym o parque)."),
        ]
        for code, name, description in data:
            self.session.add(
                WorkoutPlace(code=code, name=name, description=description)
            )
        self.session.commit()
    ...
    def seed_workout_hybrid_places(self):
        ...
        for code, name, description in data:
            self.session.add(
                WorkoutHybridPlaces(code=code, name=name, description=description)
            )
        self.session.commit()
    ...
    def seed_everyday_items(self):
        ...
        mappings = {
            EverydayItems.CHAIR: (
                "chair",
                "Silla de comedor o cualquier tipo de silla que se tenga en casa",
                "Silla,Banco",
            ),
            EverydayItems.WATER_JUGS: (
                "waterJugs",
                "Cualquier tipo de garrafón es bueno incluso si no se tiene uno de 5 o 20 litros",
                "Garrafón,Peso",
            ),
            EverydayItems.LOADED_BACKPACK: (
                "loadedBackpack",
                "Puedes cargarla de cualquier tipo de cosa en caso de no tener libros o arroz.",
                "Mochila,Peso",
            ),
            EverydayItems.TOWELS: (
                "towels",
                "Cualquier tipo de toalla de cualquier tamaño",
                "Toalla",
            ),
            EverydayItems.BROOMSTICK: (
                "broomstick",
                "O cualquier tipo de palo que te ayude a hacer ejercicio.",
                "Palo",
            ),
            EverydayItems.STAIRS: (
                "stairs",
                " o cualquier parte que esté un poco elevada que te ayude a subir y bajar.",
                "Escalón",
            ),
            EverydayItems.SOFA: ("sofa", "Sofá o Sillón", "Sofá"),
            EverydayItems.DETERGENT_BOTTLES: (
                "detergentBottles",
                "o botellas de agua",
                "Detergente,Peso",
            ),
            EverydayItems.THICK_BOOKS: (
                "thickBooks",
                "o cualquier tipo de libro",
                "Libros,Peso",
            ),
    ...
    def seed_gym_equipment(self):
        ...
        for enum_val, (code, mapping) in mappings.items():
            self.session.add(
                GymEquipmentModel(code=code, name=enum_val.value, mapping=mapping)
            )
        self.session.commit()
    ...
    def seed_home_equipment(self):
        ...
        mappings = {
            ...
            HomeEquipment.ADJUSTABLE_DUMBBELLS: (
                "adjustableDumbbells",
        ...
        for enum_val, (code, mapping) in mappings.items():
            self.session.add(
                HomeEquipmentModel(code=code, name=enum_val.value, mapping=mapping)
            )
        self.session.commit()
    ...
    def seed_outdoor_equipment(self):
        ...
        for enum_val, (code, mapping) in mappings.items():
            self.session.add(
                OutdoorEquipmentModel(code=code, name=enum_val.value, mapping=mapping)
            )
        self.session.commit()
