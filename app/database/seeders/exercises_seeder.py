...
class ExercisesSeeder(BaseSeeder):
    ...
    def _seed_base_exercises(self):
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                pattern_enum = self._normalize_pattern(
                    pattern_str, valid_patterns, code
                )
                ...
                self._associate_conditions(
                    ex.id, compat_str, caution_str, forbidden_str
                )
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if (
                    not line_str
                    or "\t" not in line_str
                    or line_str.startswith("exercise_id\t")
                ):
                    continue
                cols = [col.strip() for col in line_str.split("\t")]
                exercise_id = cols[0]
                ...
                primary_tool = cols[6]
                secondary_tool = (
                    cols[7] if len(cols) > 7 and cols[7] != "Ninguna" else None
                )
                location = cols[8]
                complexity = cols[9]
                level_str = cols[10]
                ...
                existing_ex = (
                    self.session.query(Excersice).filter(Excersice.code == code).first()
                )
                if existing_ex:
                ...
                if target_muscles:
                    for tm in [
                        m.strip() for m in target_muscles.split(",") if m.strip()
                    ]:
                ...
                if secondary_muscles:
                    for sm in [
                        m.strip() for m in secondary_muscles.split(",") if m.strip()
                    ]:
    ...
    def _normalize_pattern(
        self, pattern_str: str, valid_patterns: dict, code: str
    ) -> ExcersicePattern:
    ...
    def _associate_goals(self, ex: Excersice, goals_str: str):
        if goals_str:
            g_codes = [
                g.strip() for g in goals_str.replace("-", ",").split(",") if g.strip()
            ]
    ...
    def _associate_equipment(self, ex_id: int, primary_tool: str, secondary_tool: str):
        if (
            primary_tool
            and primary_tool not in ["Ninguna", "Ninguno", "Ningún", ""]
            and primary_tool in self.equip_map
        ):
            self.session.add(
                ExcersiceEquipment(
                    excersice_id=ex_id,
        ...
        if (
            secondary_tool
            and secondary_tool not in ["Ninguna", "Ninguno", "Ningún", ""]
            and secondary_tool in self.equip_map
        ):
            self.session.add(
                ExcersiceEquipment(
                    excersice_id=ex_id,
    ...
    def _associate_conditions(
        self, ex_id: int, compat_str: str, caution_str: str, forbidden_str: str
    ):
    ...
    def _associate_muscles(self, ex_id: int, muscle_group: str, pattern: str):
        ...
        for idx, m_name in enumerate(mg_mapping.get(muscle_group, [])):
            ...
            if muscle:
                self.session.add(
                    ExcersiceMuscle(
                        excersice_id=ex_id, muscle_id=muscle.id, is_primary=(idx == 0)
                    )
                )
