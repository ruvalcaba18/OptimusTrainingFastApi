...
class ExerciseSelector:
    def select_exercises(self, db: Session, user: User) -> list[dict[str, Any]]:
        ...
        user_condition_ids = [c.id for c in user.pathologies] + [
            c.id for c in user.diseases
        ]
        ...
        for ex in exercises:
            ...
            is_forbidden, caution_warnings = self._evaluate_health_restrictions(
                db, ex.id, user_condition_ids
            )
            if is_forbidden:
                continue
    ...
    def _fallback_all_exercises(self, db: Session) -> list[dict[str, Any]]:
        ...
        return [
            {"exercise": ex, "caution_warnings": [], "has_caution": False}
            for ex in exercises
        ]

    def _get_exercises_by_goal(
        self, db: Session, goal_id: int | None
    ) -> list[Excersice]:
    ...
    def _is_level_compatible(
        self, ex_level: str, user_level_value: ExerciseLevel
    ) -> bool:
    ...
    def _evaluate_health_restrictions(
        ...
        relations = (
            db.query(ExcersiceCondition)
            .filter(ExcersiceCondition.excersice_id == ex_id)
            .all()
        )
    ...
    def _has_required_equipment(
        self, db: Session, ex_id: int, user_equip_ids: set[int]
    ) -> bool:
        ex_equip_relations = (
            db.query(ExcersiceEquipment)
            .filter(ExcersiceEquipment.excersice_id == ex_id)
            .all()
        )
        ...
        for eq_rel in ex_equip_relations:
            if eq_rel.is_primary:
                eq_item = (
                    db.query(Equipment)
                    .filter(Equipment.id == eq_rel.equipment_id)
                    .first()
                )
