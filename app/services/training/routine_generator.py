...
class RoutineGenerator:
    ...
    def generate_routine(
        db: Session, user: User, day: int | None = None
    ) -> dict[str, Any]:
        ...
        num_exercises = RoutineGenerator._determine_exercise_count(
            user.session_duration_code
        )
        sample_size = min(len(viable_exercises), num_exercises)

        selected_sample = RoutineGenerator._sample_exercises(
            user.id, viable_exercises, sample_size, day
        )

        routine_exercises = RoutineGenerator._format_selected_exercises(
            selected_sample, matrix_reps=rules["reps"]
        )
    ...
    def _get_programming_rules(db: Session, user: User) -> dict[str, Any]:
        ...
        return {
            ...
            "method_name": matrix_rule.method.name
            if (matrix_rule and matrix_rule.method)
            else "Series Tradicionales",
        }
    ...
    def _sample_exercises(
        user_id: int, exercises: list[dict[str, Any]], count: int, day: int | None
    ) -> list[dict[str, Any]]:
        if day is not None:
    ...
    @staticmethod
    def _format_selected_exercises(
        selected_sample: list[dict[str, Any]], matrix_reps: str = "12 reps"
    ) -> list[dict[str, Any]]:
        routine_exercises = []

        # Ejercicios continuos por tiempo o distancia
        cardio_distance_exercises = {"Caminar", "Trotar", "Correr", "Sprint"}
        cardio_time_exercises = {
            "Saltar la cuerda",
            "Jumping Jacks",
            "High Knees",
            "Butt Kicks",
        ...
        for item in selected_sample:
            ...
            if ex_name in cardio_distance_exercises:
                ...
                custom_reps = (
                    "20-30 min / 3-5 km"
                    if ex_name == "Correr"
                    else ("15-20 min / 2-3 km" if ex_name == "Trotar" else "30-45 min")
                )
            elif ex_name in cardio_time_exercises:
                presc_type = "TIME"
                custom_reps = "45-60 seg"
            elif (
                ex_name in isometric_exercises
                or "Plancha" in ex_name
                or "Hold" in ex_name
            ):
    ...
    def _save_or_update_routine(
        db: Session, user_id: int, week: int, day: int, routine: dict[str, Any]
    ) -> None:
    ...
    def generate_and_save_monthly_routine(db: Session, user: User) -> dict[str, Any]:
        ...
        for week in [1, 2, 3, 4]:
            for day_val in days:
                ...
                RoutineGenerator._save_or_update_routine(
                    db, user.id, week, day_val, routine
                )
    ...
    @staticmethod
    def generate_and_save_daily_routine(
        db: Session, user: User, day: int
    ) -> dict[str, Any]:
    ...
    def update_user_routine(
        ...
        user: User,
        day: int,
        update_data: UserRoutineUpdateSchema,
        week: int | None = None,
    ) -> UserRoutine:
        ...
        query = db.query(UserRoutine).filter(
            UserRoutine.user_id == user.id, UserRoutine.day == day
        )
        if week is not None:
        ...
        if not existing:
            ...
            detail_msg = f"No se encontró una rutina generada para el día {day}" + (
                f" de la semana {week}." if week else "."
            )
            raise HTTPException(
    ...
    def update_or_regenerate_week_routine(
        ...
        for day_val in target_days:
            ...
            results.append({"week": week, "day": day_val, "routine": routine})
    ...
    def get_monthly_plan(db: Session, user: User) -> dict[str, Any]:
        routines = (
            db.query(UserRoutine)
            .filter(UserRoutine.user_id == user.id)
            .order_by(UserRoutine.week, UserRoutine.day)
            .all()
        )
    ...
    def get_routine_for_day(
        db: Session, user: User, week: int, day: int
    ) -> UserRoutine:
        routine = (
            db.query(UserRoutine)
            .filter(
