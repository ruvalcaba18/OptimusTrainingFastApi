...
class TrainingService:
    ...
    def assign_athlete(db: Session, coach_id: int, athlete_id: int) -> CoachAthlete:
        ...
        active_count = (
            ...
            .filter(CoachAthlete.coach_id == coach_id, CoachAthlete.is_active == True)
            .count()
        )
        if active_count >= 10:
        ...
        existing = (
            db.query(CoachAthlete)
            .filter(
                CoachAthlete.coach_id == coach_id, CoachAthlete.athlete_id == athlete_id
            )
            .first()
        )
    ...
    @staticmethod
    def create_plan(
        db: Session, coach_id: int, plan_in: TrainingPlanCreate
    ) -> TrainingPlan:
        ...
        if existing:
            raise ValueError(
                f"Ya existe un plan para el mes {plan_in.month}/{plan_in.year}."
            )
    ...
    def update_plan_status(
        db: Session, plan: TrainingPlan, status: PlanStatus
    ) -> TrainingPlan:
        plan.status = status
        db.add(plan)
        db.flush()
    ...
    @staticmethod
    def add_workout(
        db: Session, plan_id: int, workout_in: DailyWorkoutCreate
    ) -> DailyWorkout:
        db_workout = DailyWorkout(
            plan_id=plan_id, date=workout_in.date, status=WorkoutStatus.PENDING
        )
        db.add(db_workout)
        db.flush()
    ...
    def update_workout_exercises(
        ...
        db_workout = (
            db.query(DailyWorkout).filter(DailyWorkout.id == workout_id).first()
        )
        if not db_workout:
            return None

        db.query(ExerciseDetail).filter(
            ExerciseDetail.workout_id == workout_id
        ).delete()
    ...
    def can_see_next_workout(db: Session, plan_id: int, current_date: date) -> bool:
        ...
        last_workout = (
            ...
            .filter(DailyWorkout.plan_id == plan_id, DailyWorkout.date < current_date)
