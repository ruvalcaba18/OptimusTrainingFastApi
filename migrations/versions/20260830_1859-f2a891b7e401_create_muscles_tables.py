...
down_revision: str | Sequence[str] | None = "c912d5a06ceb"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["excersice_id"], ["excersices.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(["muscle_id"], ["muscles.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        op.f("ix_excersice_muscles_id"), "excersice_muscles", ["id"], unique=False
    )
