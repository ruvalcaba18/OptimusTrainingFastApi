...
down_revision: str | Sequence[str] | None = "f2a891b7e401"
...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_body_parts_name_en"), "body_parts", ["name_en"], unique=True
    )
    ...
    op.create_index(
        op.f("ix_exercise_types_code"), "exercise_types", ["code"], unique=True
    )
    op.create_index(
        op.f("ix_exercise_types_id"), "exercise_types", ["id"], unique=False
    )
    op.create_index(
        op.f("ix_exercise_types_name_en"), "exercise_types", ["name_en"], unique=True
    )

    # 3. Update Equipment table with name_es and image_url
    op.add_column(
        "equipment", sa.Column("name_es", sa.String(length=100), nullable=True)
    )
    op.add_column(
        "equipment", sa.Column("image_url", sa.String(length=500), nullable=True)
    )
