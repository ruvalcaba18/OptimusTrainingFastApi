...
down_revision: str | Sequence[str] | None = "a3c982d41b55"
...
def upgrade() -> None:
    op.add_column(
        "excersices", sa.Column("exercise_id", sa.String(length=100), nullable=True)
    )
    op.add_column(
        "excersices", sa.Column("image_url", sa.String(length=500), nullable=True)
    )
    op.create_index(
        op.f("ix_excersices_exercise_id"), "excersices", ["exercise_id"], unique=True
    )
